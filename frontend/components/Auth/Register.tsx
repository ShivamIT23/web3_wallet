...
export default function RegisterPage() {
    ...
    const handleRegister = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (isExtension) {
                // --- LOGIC A: EXTENSION MODE ---
                const res = await fetch("https://wallet-backend.shivam23.me/register", {
                    method: "POST",
                    body: JSON.stringify({ email, password }),
                });

                const data = await res.json();
                console.log("Extension register response:", data);

                if (res.ok && data.token) {
                    // Check if we are in the Chrome environment
                    // @ts-expect-error Chrome types are not available in standard Next.js env
                    if (typeof chrome !== "undefined" && chrome.storage && chrome.storage.local) {
                        ...
                        // Fallback for testing extension logic in a regular browser
                ...
                // --- LOGIC B: WEBSITE MODE ---
                // For website, we might want to register via API then auto-login or redirect to login.
                // Assuming a /api/register route or similar exists or we use the backend directly.
                // For now, let's try calling backend directly as we don't have a specific NextAuth provider for custom signup easily without custom adapter.
                // OR we can just simulate success and redirect to login.
                
                // Let's call the backend directly for registration as well for consistency if possible, 
                // but NextAuth credentials provider is usually for login.
                // We'll implemented a direct backend call for registration.
                
                 const res = await fetch("https://wallet-backend.shivam23.me/register", { // URL placeholder
                    method: "POST",
